...
* **Revisão adversarial de design, antes de implementar (spec ou plano, nunca código já pronto):** pra redesenho arquitetural de verdade (critério da seção 1 do `sira-claude-workflow`: decisão de produto nova, arquivos com lógica relacionada, migration), rodar `/codex:adversarial-review` sobre a **spec ou o plano escrito**, antes de despachar a Task 1, questionando de propósito:
  - premissa que parece óbvia mas nunca foi testada de verdade (ex.: "o cooldown de 1 minuto por candidatura garante mesmo que não existe corrida entre dois envios simultâneos?");
  - estado otimista marcado como "concluído" antes de confirmar que o efeito colateral externo (envio de mensagem, chamada de API, gravação em serviço de terceiro) realmente aconteceu — se ele puder falhar depois, o que reverte o estado?
  - invariante que precisa valer em mais de um arquivo ou task ao mesmo tempo (dois lugares que precisam concordar sobre o mesmo predicado).
  Comando sempre com foco de texto explícito, nunca genérico — "revise isso" sem pergunta concreta desperdiça o que o comando tem de diferente do `/codex:review`: `/codex:adversarial-review --base <SHA-antes-da-Task-1> "questione [premissa/trade-off específico da spec ou do plano]"`. Roda uma vez, sobre o documento, não sobre código (código ainda não existe nesse ponto do fluxo).
  **Não usar** pra task mecânica, nem como substituto de `/codex:review` depois que o código já existe — são momentos diferentes e não se sobrepõem: adversarial questiona a decisão **antes** de escrever, review acha bug **depois** de escrever.

* **Revisão de branch inteira:** é a **prioridade 1** da revisão final de `subagent-driven-development` (seção 2.3 da skill) — substitui a revisão completa em Claude no caso normal, não soma com ela. Claude entrega o comando `/codex:review --base <SHA-de-início-do-plano> --model gpt-5.6-sol` sozinho nesse ponto do fluxo, um disparo só, tier mais capaz do Codex (`gpt-5.6-sol`, seção 7). **Nunca `--scope branch` sozinho, sem `--base` explícito, em branch de longa duração como `auto-admission`** — `--scope branch` sem `--base` compara contra o merge-base com `main`, o que traz a branch inteira desde que nasceu. `--base` é sempre o SHA imediatamente anterior à Task 1 do plano que está fechando — não o merge-base com `main`, não um SHA genérico. Claude dividido por área (Claude/Opus lendo o diff em partes) só entra como fallback, se Codex estiver indisponível ou você pedir pra pular. Fora desse contexto, mesmo comando por sua iniciativa a qualquer momento.
